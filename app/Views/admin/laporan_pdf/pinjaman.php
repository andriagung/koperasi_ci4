    ...
    <table>
        ...
        <tbody>
            ...
            foreach($pinjaman as $p): 
                $totalPinjaman += $p['nominal_pengajuan'];
            ?>
            <tr>
                <td class="text-center"><?= date('d-m-Y', strtotime($p['tanggal_pengajuan'])) ?></td>
                <td class="text-center"><?= esc($p['nip'] ?? '') ?></td>
                <td><?= esc($p['nama_lengkap'] ?? '') ?></td>
                <td class="text-right"><?= number_format($p['nominal_pengajuan'] ?? 0, 0, ',', '.') ?></td>
                <td class="text-center"><?= $p['tenor_bulan'] ?? '' ?> Bulan</td>
                <td class="text-center"><?= esc($p['status_pengajuan'] ?? '') ?></td>
            </tr>
            <?php endforeach; ?>
            <?php if(count($pinjaman) == 0): ?>
            <tr>
                <td colspan="6" class="text-center">Tidak ada data transaksi pinjaman pada periode ini.</td>
            </tr>
            <?php else: ?>
            <tr>
                <td colspan="3" class="text-right" style="font-weight: bold;">TOTAL KESELURUHAN</td>
                <td class="text-right" style="font-weight: bold;"><?= number_format($totalPinjaman ?? 0, 0, ',', '.') ?></td>
                <td colspan="2"></td>
            </tr>
            <?php endif; ?>
        </tbody>
